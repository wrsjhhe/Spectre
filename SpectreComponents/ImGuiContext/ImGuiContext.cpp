...
void ImageGUIContext::Init(const std::string& font,uint32_t width,uint32_t height)
{
    ImGui::CreateContext();
...
void ImageGUIContext::PreparePipeline(VkRenderPass renderPass, int32_t subpass, VkSampleCountFlagBits sampleCount)
{
...
void ImageGUIContext::PreparePipelineResources()
{
    ...
    setLayoutCreateInfo, VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
...
void ImageGUIContext::Resize(uint32_t width, uint32_t height)
{
    ImGuiIO& io = ImGui::GetIO();
    io.DisplaySize = ImVec2((float)(width), (float)(height));
}
...
void ImageGUIContext::BindDrawCmd(const VkCommandBuffer& commandBuffer, const VkRenderPass& renderPass, int32_t subpass, VkSampleCountFlagBits sampleCount)
{
    ImDrawData* imDrawData = ImGui::GetDrawData();
