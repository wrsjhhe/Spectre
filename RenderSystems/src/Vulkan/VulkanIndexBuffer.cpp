...
VulkanIndexBuffer::VulkanIndexBuffer(std::vector<uint32_t> indices, VkIndexType indexType):
...
VulkanIndexBuffer::VulkanIndexBuffer(std::vector<uint16_t> indices, VkIndexType indexType):
	m_IndexCount(indices.size()),
	m_IndexType(indexType)
{
	uint32_t bufferSize = indices.size() * sizeof(uint16_t);
	m_HostBuffer = VulkanBuffer::Create(indices.size() * sizeof(uint16_t),
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, indices.data());
}
